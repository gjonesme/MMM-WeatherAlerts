		...
		this.fetchData(this.getUrl())
			.then((data) => {
				...
				if (this.config.weatherEndpoint === "/onecall") {
					currentWeatherAlerts = this.generateWeatherAlertObjectsFromOnecall(data).alerts;
					this.setFetchedLocation(`${data.timezone}`);
				} else {
					Log.info(`Alerts can only be fetched from /onecall endpoint, found endpoint ${this.configu.weatherEndpoint}`);
	...
	fetchOnecall(data) {
		...
		if (data.hasOwnProperty("alerts")) {
			for (const alert of data.alerts) {
				...
				weatherAlert.end = moment.unix(alert.end).utcOffset(data.timezone_offset / 60);
				...
				weatherAlert.start = moment.unix(alert.start).utcOffset(data.timezone_offset / 60);
